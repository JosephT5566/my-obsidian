...
topics: [ai, gcf, gcs, jwt, frontend]
---
...
### Client 使用 Signed URL 直接上傳 GCS

- GCF 先驗證 Supabase JWT，再發行短效 Signed URL。
- Client 直接 `PUT` 圖片到 GCS。
- Client 將上傳後的 Object URL 交給 AI analysis GCF。

## Decision

採用 GCF + GCS + AI API 的分離 Pipeline，並讓 Travel Split 與 Expense AI 使用不同 GCF，以分離用途、設定與帳戶資源。

## Consequences

流程分成：

1. **Upload**：App 向 GCF 取得約 10 分鐘有效的 Signed URL，再直接上傳 GCS。
...
驗證使用 Supabase JWKS 的非對稱簽章，GCF 可從公開端點取得驗證金鑰，不需持有 Legacy symmetric JWT secret。Service account 只授予建立 Signed URL 與讀取必要 Object 的最低權限。
