...
Foreign key 保存 entity relationship，例如 expense 目前屬於哪個 category。欄位型別、`CHECK` 與 [[Database Unique Constraint|UNIQUE constraint]] 可由 PostgreSQL 對所有寫入路徑一致執行，不會被 SQL console、migration 或 background job 繞過。這些 constraint 保護 data integrity；「誰能操作哪些 rows」則是 [[PostgreSQL Row-Level Security]] 的 authorization responsibility。
