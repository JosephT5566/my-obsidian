...
type: concept
status: growing
topics: [houzz, kubernetes, containers, networking, debugging]
created: "2026-07-14"
---
...
在 [[Houzz]] 的具體使用案例中，[[C2 Development on Kubernetes]] 會先取得 EKS context，再進入 Codepath pod 的 `c2web` 或 `c2thrift` container；[[Redis Debugging]] 與 [[Email Template Testing]] 也會使用 debug pod 進行受控診斷。Kubernetes 頁保留通用資源模型，這些 Workflow 頁則保留環境特有步驟。
