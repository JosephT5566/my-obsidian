...
對 `(user_id, endpoint, idempotency_key)` 建立 [[Database Unique Constraint|unique constraint]]。不要使用「先 `SELECT`、不存在才 `INSERT`」的 check-then-act；多個 instance 可能同時看到不存在。應以 atomic insert 或 `INSERT ... ON CONFLICT DO NOTHING RETURNING ...` 讓 database 仲裁 winner。
...
若外部 provider 也有 side effect，internal key 不能自動保護該 boundary。應把穩定 job ID 傳為 provider [[Idempotency Key|idempotency key]]，或保存 provider async job ID 供 reconciliation。

 > Reconciliation（對帳／狀態核對）**是定期比較「我們資料庫記錄的狀態」與「外部 provider 實際記錄的狀態」，找出不一致並修正。

 處理像是這種狀況
```
Provider 已成功完成解析
→ Worker 在寫回 database 前 crash
→ 本地顯示 processing
→ Provider 實際上是 completed
```
**reconciliation worker** 會定期尋找長時間未完成的工作，查詢 Parsing job table 裡的 Pending state，再向 provider 詢問狀態，並回頭更新本地狀態。
如果 provider 不提供狀態查詢、request history 或可搜尋的 idempotency key，reconciliation 就很難自動完成。此時只能依靠 provider 控制台、帳單紀錄或人工核對，也無法完全避免重複呼叫與額外費用。
...
- Result table 對 `job_id` 設 [[Database Unique Constraint|unique constraint]]，只能避免本地重複結果，不能阻止 provider 被重複呼叫。
